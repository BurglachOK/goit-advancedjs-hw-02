...
feedbackFormEl.addEventListener('submit', event => {
  ...
  if (usermail === '' || usermessage === '') {
    ...
    // вивід у консоль об’єкт formData
    console.log(formData);

    // очищення локального сховища, об’єкт formData і поля форми
...
feedbackFormEl.addEventListener('input', event => {
  ...
  // додання об'кт у localStorage
  saveToLocalStorage(feedbackFormState, formData);
});

// При завантаженні сторінки перевірка, чи є дані у локальному сховищі.
